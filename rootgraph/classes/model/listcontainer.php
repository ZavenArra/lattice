...
class Model_ListContainer extends Model_Object {
   ...
   public function getSortDirection(){
      ...
      if(!$this->_sortDirection){
          $this->_sortDirection = lattice::config('objects', sprintf('//list[@family="%s"]', $this->objecttype->objecttypename))->item(0)->getAttribute('sortDirection');   
      }
   ...
   public function getConfig(){
      if (!$this->_xml_config) {
         $xPathLookup = sprintf('//list[@family="%s"]', $this->objecttype->objecttypename);
