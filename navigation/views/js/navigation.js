...
mop.modules.navigation.Tier = new Class({
   ...
   onDrawerMouseLeave: function( e ){
      ...
      this.drawer.morph( {
         'top': this.drawer.retrieve( "initTop" )
      } );
   },
   ...
   addPlaceHolder: function( name, whichTier ){
      ...
      var nodesElement = this.element.getElement("ul.nodes");
      this.addObjectPosition = 'bottom'; // should read from class

      nodeTitle.inject( placeHolder );
      clearBoth.inject( placeHolder );
      placeHolder.inject( nodesElement, this.addObjectPosition );
  
      if( !this.slideTier) this.slideTier = new Fx.Scroll( nodesElement );
      if(this.addObjectPosition == 'top'){
         this.slideTier.toTop();
      } else {
         this.slideTier.toBottom();
      }
