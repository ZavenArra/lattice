...
Class Lattice_Frontend_Associator_Radios {

  public static function make_pool($associated_views, $pool_views)
  {
    if (count($associated_views))
    {
      $keys = array_map(array('Frontend_Associator_Radios','title_index'), $associated_views);
      $associated_views = array_combine($keys, $associated_views);
    }
    $pool_views = array_combine( array_map(array('Frontend_Associator_Radios','title_index'), $pool_views),  $pool_views);
    ...
    array_walk($pool_views, array('Frontend_Associator_Radios', 'set_unique_element_id'));
