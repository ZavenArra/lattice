...
class Model_Object extends ORM implements arrayaccess {
	...
	protected $_belongs_to = array('objecttype' => array());
  
	protected $_has_one = array('objecttype' => array());
	
	protected $_has_many = array(
								'tag' => array('model' => 'tag', 'through' => 'objects_tags', 'foreign_key' => 'object_id'),
								'roles' => array('model' => 'role', 'through' => 'objects_roles', 'foreign_key' => 'object_id'),
								'users' => array('model' => 'user', 'through' => 'objects_users', 'foreign_key' => 'object_id') 
							);
	...
	*/
	public static function create_slug($title_or_slug=NULL, $for_page_id=NULL)
	{
		...
		if ($title_or_slug!=NULL)
		{
			...
			$check_slug = Graph::object()->where('slug', '=', $slug);
			...
			$check_slug = Graph::object()->where('slug', 'REGEXP',  '^'.$slug.'[0-9]*$')->order_by("slug", 'DESC');
    ...
    */
	...
	/*
	*resize an image from the center
	*/
	public static function resize_image($original_filename, $new_filename, $width, $height, $force_dimension='width', $crop='FALSE', $aspect_follows_orientation='false')
	{
    ...
    */
	...
	public function translate($language_code)
    {
		...
		if ( ! $rosetta_id)
		{
			throw new Kohana_Exception('No Rosetta ID found for object during translation with object_id :object_id', array(':object_id'=>$rosetta_id));
		}
		...
		{
			throw new Kohana_Exception('No translated object available for object_id :id with language :language', array(':id'=>$object_id, ':language'=>$language_code));
    ...
    {
		$fields = ORM::Factory('objectmap')->where('objecttype_id', '=', $this->objecttype->id)->find_all();
    ...
    */
    ...
    public function verify_image($field, $tmp_name)
    {
		...
		{
			$result  = 	array('result' => 'failed', 'error' => 'Image height less than minimum height');
			return $result;
		}
      
		if (isset($parameters['minwidth']) AND $origwidth < $parameters['minwidth'])
		{
			$result = array('result' => 'failed', 'error' => 'Image width less than minimum width');
			return $result;
		}
    }
    ...
    */
    ...
    public function content_filter($wheres)
    {
		...
		$driver_info = array('driver' => 'mysql', 'table_name' => 'contents');
    ...
    {
		$res = ORM::Factory('objecttype')->where('nodeType', '=', 'container')->find_all();
		$t_ids = array();
    ...
    */
    ...
    public function is_within_sub_tree($object_id, $lattice='lattice')
    {
		...
		{
			throw new Kohana_Exception('Checking for object subtree of object that is not in database: :objectid', array(':objectid'=>$object_id));
		}
		$object = $this;
    ...
    public function adjacent_record($sort_field, $direction, $current_id = NULL, $lattice='lattice' )
    {
		...
		if ($this->is_table_column($sort_field))
		{
			...
			$query->where($sort_field, $inequality, $sort_value)->order_by($sort_field, $order);
		} 
    ...
    public function get_translated_object($language_id)
    {
		...
		if ( ! $translated_object->loaded())
		{
			throw new Kohana_Exception('No matching translated object for rosetta :rosetta and language :language', array(':rosetta'=>$this->rosetta_id, ':language'=>$language_id));
		}
		return $translated_object;
    }
